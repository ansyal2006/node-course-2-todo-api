...
const users = [
  {
    ...
    tokens : [{
      access : 'auth',
      token : jwt.sign({ _id : newid1.toHexString(), access : 'auth'}, process.env.JWT_SECRET).toString()
     }]
  },
  {
    _id : newid2,
    ...
    tokens : [{
      access : 'auth',
      token : jwt.sign({ _id : newid2.toHexString(), access : 'auth'}, process.env.JWT_SECRET).toString()
     }]
  }
];
